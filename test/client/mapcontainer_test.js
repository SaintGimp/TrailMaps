...
define(["jquery", "/test/client/testableMapContainer.js"], function ($, testableMapContainer) {
  ...
  function initialize(mapName, done) {
    ...
    initializeDOM();
    testableMapContainer.create(mapName, server).done(function (newMapContainer) {
  ...
  describe("Map container", function () {
    ...
    describe("Switching to another map type", function () {
      before(function (done) {
        initialize("bing", function () {
          mapContainer.showingMap("google").done(function () {
            server.respond();
            done();
          });
        });
    ...
    describe("Zooming in on the map", function () {
      ...
      it("should load a higher detail level", function () {
        expect(mapContainer.requestedDetailLevel).to.equal(6);
      });
      ...
      before(function (done) {
        initialize("bing", function () {
          mapContainer
            ...
            .done(function () {
              mapContainer.showingMap("bing").done(function () {
                server.respond();
                done();
              });
            });
